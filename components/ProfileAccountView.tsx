...
import { 
    ...
    Image as ImageIcon, RotateCcw, AlertCircle
} from 'lucide-react';
...
interface ProfileAccountViewProps {
    ...
    onManageCategories: () => void; // NEW PROP
    receiptCount: number; // For storage calculation
}
...
const AvatarCameraView = ({ onCapture, onCancel }: { onCapture: (img: string) => void, onCancel: () => void }) => {
    ...
    const handleCapture = () => {
        if (videoRef.current) {
            ...
            // Create a 512x512 square avatar
            ...
            if (ctx) {
                ...
                // Draw and flip horizontally for "mirror" effect natural to selfies
                ctx.translate(512, 0);
                ctx.scale(-1, 1);
    ...
    if (permissionDenied) {
        return (
            <div className="fixed inset-0 z-[80] bg-black flex flex-col items-center justify-center p-6 text-white animate-fade-in">
                ...
                <div className="flex flex-col gap-3 w-full max-w-sm">
                    <button onClick={onCancel} className="w-full py-3 rounded-xl bg-white text-black font-semibold">
                        Close
                    </button>
                </div>
            </div>
        ...
        <div className="fixed inset-0 z-[80] bg-black flex flex-col animate-fade-in">
            ...
            <div className="flex-1 relative overflow-hidden flex items-center justify-center">
                ...
                <div className="absolute inset-0 bg-black/50">
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 rounded-full bg-transparent border-4 border-white/30 shadow-[0_0_0_9999px_rgba(0,0,0,0.5)] overflow-hidden">
                         {/* Optional Grid or Guide */}
...
const ProfileAccountView: React.FC<ProfileAccountViewProps> = ({ 
    ...
    const ListItem = ({ icon: Icon, color, label, value, onClick, isDestructive = false, hasToggle = false, toggleValue = false, onToggle = () => {} }: any) => (
        ...
        >
            <div className="flex items-center gap-3">
                ...
                <span className={`text-[16px] font-medium ${isDestructive ? 'text-red-600' : 'text-neutral-900'}`}>
                    {label}
                </span>
            </div>
    ...
    const SectionHeader = ({ title }: { title: string }) => (
        <h3 className="px-4 pb-2 pt-6 text-xs font-semibold text-neutral-500 uppercase tracking-wider">
            {title}
        </h3>
    );
    ...
    return (
        <div className="flex flex-col h-full bg-ios-bg">
            {/* --- Header --- */}
            <div className="bg-ios-card px-4 pt-12 pb-6 border-b border-ios-separator/20 sticky top-0 z-20 shadow-sm">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                        ...
                        {userProfile.isPro && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-gradient-to-r from-amber-100 to-orange-100 text-orange-600 text-[10px] font-bold uppercase tracking-wide mt-1 border border-orange-200">
                                <Zap size={10} fill="currentColor" /> Pro Account
                            </span>
                        )}
                    </div>
                    <button 
                        onClick={() => setIsEditingProfile(true)}
                        className="bg-neutral-100 hover:bg-neutral-200 text-neutral-900 px-4 py-2 rounded-full text-sm font-semibold transition-colors"
                    >
                        Edit
                    </button>
                </div>
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar pb-32">
                ...
                <div className="bg-white border-y border-ios-separator/30">
                    ...
                    <div className="h-px bg-ios-separator/30 ml-14" />
                    <ListItem 
                        icon={Moon} color="bg-neutral-700" label="Reduce Motion" 
                        hasToggle toggleValue={userProfile.reduceMotion} 
                        onToggle={(val: boolean) => onUpdateProfile({...userProfile, reduceMotion: val})} 
                    />
                </div>
                ...
                <div className="bg-white border-y border-ios-separator/30">
                    ...
                    <div className="p-4">
                        ...
                        <input 
                            ...
                            value={userProfile.ocrThreshold}
                            onChange={(e) => onUpdateProfile({...userProfile, ocrThreshold: parseFloat(e.target.value)})}
                    ...
                    <div className="h-px bg-ios-separator/30 ml-14" />
                    <ListItem 
                        icon={FileText} color="bg-teal-500" label="Auto-Categorize" 
                        hasToggle toggleValue={userProfile.autoCategorize} 
                        onToggle={(val: boolean) => onUpdateProfile({...userProfile, autoCategorize: val})} 
                    />
                    <div className="h-px bg-ios-separator/30 ml-14" />
                    <ListItem 
                        icon={Camera} color="bg-yellow-500" label="Auto-Crop Receipts" 
                        hasToggle toggleValue={userProfile.autoCrop} 
                        onToggle={(val: boolean) => onUpdateProfile({...userProfile, autoCrop: val})} 
                    />
                </div>
                ...
                <div className="bg-white border-y border-ios-separator/30">
                     <ListItem 
                        icon={Smartphone} color="bg-pink-500" label="Haptic Feedback" 
                        hasToggle toggleValue={userProfile.hapticsEnabled} 
                        onToggle={toggleHaptic} 
                    />
                    <div className="h-px bg-ios-separator/30 ml-14" />
                    <ListItem 
                        icon={Bell} color="bg-red-500" label="Alerts & Summaries" 
                        hasToggle toggleValue={userProfile.notifications.ocr} 
                        onToggle={(val: boolean) => onUpdateProfile({...userProfile, notifications: {...userProfile.notifications, ocr: val}})} 
                    />
                </div>
                ...
                <div className="bg-white border-y border-ios-separator/30">
                    <div className="p-4 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <span className="block text-[16px] font-medium text-neutral-900">Storage Used</span>
                                <span className="block text-xs text-neutral-400">{(receiptCount * 0.15).toFixed(1)} MB estimated</span>
                            </div>
                        </div>
                    ...
                    <div className="h-px bg-ios-separator/30 ml-14" />
                    <ListItem 
                        icon={Trash2} color="bg-red-100" label="Delete All Receipts" 
                        isDestructive onClick={() => setShowDeleteConfirm(true)}
                    />
                </div>
                ...
                <div className="bg-white border-y border-ios-separator/30 mb-8">
                    ...
                    <div className="h-px bg-ios-separator/30 ml-14" />
                    <ListItem icon={Info} color="bg-neutral-400" label="About" value="v1.2.0 (Build 44)" />
                </div>

                <div className="px-4 pb-6">
                    <button 
                        onClick={onSignOut}
                        className="w-full py-3 rounded-xl bg-neutral-200 text-neutral-900 font-semibold flex items-center justify-center gap-2 active:bg-neutral-300 transition-colors"
                    >
                        ...
                        <div className="p-6 space-y-6 bg-ios-bg pb-12">
                            <div className="flex flex-col items-center gap-3">
                                <div 
                                    ...
                                    className="w-24 h-24 rounded-full bg-neutral-200 border-4 border-white shadow-md flex items-center justify-center overflow-hidden relative group cursor-pointer ios-active"
                                >
                                    {editForm.avatar ? (
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="block text-xs font-semibold text-ios-gray uppercase mb-1">Display Name</label>
                                    <input 
                                        type="text" 
                                        value={editForm.name}
                                        onChange={e => setEditForm({...editForm, name: e.target.value})}
                                        className="w-full p-3 rounded-xl border border-neutral-200 bg-white focus:outline-none focus:border-ios-blue"
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-semibold text-ios-gray uppercase mb-1">Email Address</label>
                                    <input 
                                        type="email" 
                    ...
                    <div className="w-full max-w-md p-4 animate-slide-up">
                        <div className="bg-white/90 backdrop-blur-md rounded-xl overflow-hidden shadow-lg mb-2">
                            <button 
                                ...
                                className="w-full p-4 text-center text-ios-blue font-medium text-lg border-b border-neutral-200/50 active:bg-neutral-100 transition-colors"
                            >
                                Choose from Library
                            </button>
                            <button 
                                ...
                                className="w-full p-4 text-center text-ios-blue font-medium text-lg active:bg-neutral-100 transition-colors"
                            >
                                Take Headshot
                            </button>
                        </div>
                        <button 
                            ...
                            className="w-full p-4 bg-white rounded-xl text-ios-blue font-bold text-lg shadow-lg active:bg-neutral-100 transition-colors"
                        >
                            Cancel
                        </button>
                    </div>
            ...
            {showCamera && (
                <AvatarCameraView 
                    onCapture={(img) => {
                        setEditForm(prev => ({ ...prev, avatar: img }));
                        setShowCamera(false);
                    }}
                ...
                <div className="fixed inset-0 z-[70] flex items-center justify-center p-6">
                    ...
                    <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl relative animate-scale-in text-center">
                        ...
                        <p className="text-neutral-500 mb-6 text-sm leading-relaxed">
                            This action cannot be undone. All your receipt images and data will be permanently removed from this device.
                        </p>
                        <div className="flex gap-3">
                            <button 
                                onClick={() => setShowDeleteConfirm(false)}
                                className="flex-1 py-3 rounded-xl font-semibold bg-neutral-100 text-neutral-900 hover:bg-neutral-200"
                            >
                                Cancel
                            </button>
                            <button 
                                ...
                                className="flex-1 py-3 rounded-xl font-semibold bg-red-600 text-white hover:bg-red-700"
                            >
                                Delete All
                            </button>
                        </div>
